...
- 2026-03-04 – Admin-Druckjobs um `Löschen` erweitert: In `/admin/?tab=jobs` gibt es neben `Abbrechen` jetzt `Löschen` für `canceled|failed_hard|error|done`. Beim Löschen wird der DB-Job entfernt; zugehörige Printfiles werden nur innerhalb `data/printfiles` gelöscht (Pfadgrenzen-Check).
- 2026-03-04 – Supervisor/Worker stabilisiert: `start.ps1` triggert `print_worker.php run` jetzt in jedem Supervisor-Zyklus (nicht mehr abhängig vom Pending-Count). `import/print_worker.php` setzt festhängende `sending`-Jobs (>30s ohne Spool-ID) wieder auf `queued`, berechnet Retry-Zeitstempel auf aktuellem Fehlerzeitpunkt und behandelt `spooled -> spool_job_missing` als `done` statt Endlos-Requeue.
