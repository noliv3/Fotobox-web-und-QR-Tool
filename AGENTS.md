...
- 2026-03-04 – Print-Worker Diagnose- und Statusfix: `import/print_worker.php` akzeptiert `printer_status.errorState` Werte `Normal|Idle|Ready` als nicht-fehlerhaft. Submit-Fehler `VIRTUAL_PRINTER_UNSUPPORTED|PRINTER_NOT_FOUND|JOB_ID_NOT_FOUND` werden in `needs_attention` geführt (statt still wieder `queued`). `ops/print/submit_job.ps1` liefert für virtuelle Drucker (`Microsoft Print to PDF`/`OneNote`) den klaren Fehlercode `VIRTUAL_PRINTER_UNSUPPORTED`.
