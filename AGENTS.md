...
- 2026-03-04 – Mobile Order-Done 3-Step Payflow: `web/mobile/order_done.php` rendert den Zahlungsabschluss jetzt als maximal 3 klare Schritte mit umrandeten Nummern (WLAN verlassen, PayPal oeffnen, Betrag bestaetigen), inkl. grossem PayPal-Action-Button und QR-Code. Falls `orders.paypal_url` leer ist, wird ein Fallback auf `https://paypal.me/noliv3/<betrag>` genutzt. `shared/config.php` setzt `paypal_me_base_url` auf `https://paypal.me/noliv3`; `web/mobile/style.css` ergänzt passende Step-Styles.
- 2026-03-04 – Admin-Header Kamera-Preview: Neues Endpoint `web/admin/camera_preview.php` liefert ein kleines 80x80-Livebild für den Admin-Bereich über den lokalen digiCamControl-Webserver (`127.0.0.1:5513/preview.jpg`) und fällt bei Fehler auf ein eingebettetes SVG zurück. `web/admin/index.php` zeigt das Bild oben rechts im Header und refresht es alle 4 Sekunden; `web/admin/style.css` enthält das 80x80-Layout.
- 2026-03-04 – Admin-Druckjobs `Löschen` + Worker-Laufstabilität: `web/admin/index.php` ergänzt `delete_job` (nur für `canceled|failed_hard|error|done`) mit sicherem Printfile-Delete innerhalb `data/printfiles` und entfernt den DB-Job endgültig. `start.ps1` triggert `import/print_worker.php run` in jedem Supervisor-Zyklus unabhängig vom Pending-Count. `import/print_worker.php` requeued `sending`-Jobs ohne Spool-ID nach 30s, berechnet Retry-Backoff mit aktuellem Fehlerzeitpunkt und markiert `spooled`-Jobs bei fehlendem Spooler-Eintrag als `done` statt Requeue-Loop.
